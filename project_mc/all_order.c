...
void all_order(void)
{
    char class[8]={0}, order[1600]={0}, json[256]={0}, parent[256]={0};
    ...
    while(fgets(tmp,LONGEST_LINE,fp)!=NULL){
        if(tmp[0]!='\t'){
            //处理纲
            sscanf(tmp,"%s",class);
            sprintf(json,"D:/Programs/VSCode/code_field/code_c/c_multiple/script/project_mc/advancements/all/%s",class);
            if(_mkdir(json) == -1)
                perror("无法创建目录");
            sprintf(parent,"all_item:all/%s/_class",class);
        }
        else{
                    //处理目
                    ...
                    sprintf(json,"D:/Programs/VSCode/code_field/code_c/c_multiple/script/project_mc/advancements/all/%s/%s.json",class,order);
